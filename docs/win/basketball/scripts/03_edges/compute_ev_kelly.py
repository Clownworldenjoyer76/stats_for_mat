#!/usr/bin/env python3
"""Incremental/full-rebuild launcher for compute_ev_kelly_core.py."""
...
CORE_PATH=Path(__file__).with_name("compute_ev_kelly_core.py")
REAL_INPUT=Path("docs/win/basketball/01_merge/01_merguiced")
REAL_OUTPUT=Path("docs/win/basketball/03_edges/ev_kelly")
NY=ZoneInfo("America/New_York")
LEAGUES=["nba","ncaam","wnba"]; MARKETS=["moneyline","spread","total"]
def truthy(v): return str(v or "").strip().lower() in {"1","true","yes","on"}
def load_core():
    spec=importlib.util.spec_from_file_location("compute_ev_kelly_core",CORE_PATH)
    ...
    if truthy(os.getenv("BASKETBALL_FULL_REBUILD")):
        ...
        with tempfile.TemporaryDirectory(prefix="basketball_ev_") as td:
            ...
            for lg in LEAGUES:
                up=lg.upper()
                for market in MARKETS:
                    dest=REAL_OUTPUT/lg/market/f"{date}_{up}_{market}.csv"; src=out/lg/market/dest.name
