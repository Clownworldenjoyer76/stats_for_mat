...
def current_internal_season(
    ...
    local_now = (
        now
        or datetime.now(NY_TZ)
    )

    if local_now.tzinfo is None:
        local_now = local_now.replace(
            tzinfo=NY_TZ
        )
...
def load_team_map(
    ...
    if not path.exists():
        log(
            f"TEAM MAP MISSING: {path}; "
            "SDV display names will be used unchanged"
        )
        return mapping
...
def parse_event_datetime(
    ...
    if not text:
        raise ValueError(
            "SDV schedule row has blank date"
        )
...
def validate_source_schema(
    ...
    if missing:
        raise ValueError(
            f"{league}: SDV schedule schema "
            "missing required columns: "
            f"{missing}"
        )

    if league == "ncaam":
        venue_missing = sorted(
            NCAAM_REQUIRED_VENUE_COLUMNS
            - columns
        )
        ...
        if venue_missing:
            raise ValueError(
                "ncaam: SDV schedule schema "
                "missing required venue/context "
                f"columns: {venue_missing}"
            )

    if len(df) >= limit:
        raise ValueError(
            f"{league}: SDV schedule returned "
            f"{len(df)} rows at configured "
            f"limit={limit}; refusing possible "
            "truncated season"
        )


def source_team_name(
    row: pd.Series,
    side: str,
) -> str:
    for column in (
...
def canonicalize_schedule(
    ...
    *,
    ...
    for _, source in df.iterrows():
        source_season = clean_id(
            source.get("season")
        )

        if (
            source_season
            and int(source_season)
            != int(sdv_season)
        ):
            raise ValueError(
                f"{league}: SDV returned "
                f"season={source_season} while "
                "explicit mapping requires "
                f"season={sdv_season}"
            )
        ...
        if not game_id:
            raise ValueError(
                f"{league}: SDV schedule row "
                "has blank game_id"
            )

        event_value = source.get(
            "date"
        )
        ...
        game_date = event_dt.strftime(
            "%Y_%m_%d"
        )

        game_time = event_dt.strftime(
            "%I:%M %p"
        )
        ...
        if (
            ...
            raise ValueError(
                f"{league}: SDV game_id={game_id} "
                "maps to multiple identities: "
                f"{old_identity} vs {identity}"
            )

        old_game_id = (
            seen_by_identity.get(
                identity
            )
        )

        if (
            old_game_id is not None
            and old_game_id != game_id
        ):
            raise ValueError(
                f"{league}: one SDV identity "
                "has multiple game_ids: "
                f"{identity} -> {old_game_id} "
                f"vs {game_id}"
            )
...
def build_league(
    ...
    *,
    ...
    log(
        "FETCH START | "
        f"league={cfg['label']} "
        f"internal_season={internal} "
        f"sdv_season={mapped_sdv_season}"
    )
    ...
    try:
        ...
        if (
            ...
            log(
                "FETCH WARNING | "
                f"league={cfg['label']} "
                "using valid cached canonical "
                f"schedule {path} because "
...
def build_current_canonical_games(
    ...
    log(
        f"STATUS: SUCCESS | files={len(paths)}"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Build SDV-backed canonical "
            "basketball schedules/game IDs."
        )
    )
    ...
    parser.add_argument(
        ...
        action="append",
        help=(
            "League to build; may be repeated. "
            "Default: all leagues."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "Explicit internal season. "
            "Only valid when exactly one "
            "--league is supplied."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail if the SDV refresh fails "
            "even when a valid cached file exists."
        ),
    )
