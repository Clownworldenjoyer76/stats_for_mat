...
def apply_exposure_limits(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in out.iterrows():
        ...
        if (
            str(row.get('league_lower') or row.get('league') or '').strip().lower() == 'nba'
            and str(row.get('market_type') or '').strip().lower() == 'moneyline'
        ):
            stake_multiplier = 1.0

        fractional, individual_capped, requested = requested_stake(
            raw_kelly,
            stake_multiplier
        )
